...
As data platforms evolve from traditional data warehouses to open lakehouse architectures, one requirement has remained constant: the need for **stable, unique surrogate keys**. With **Fabric Runtime 2.0**, built on **Apache Spark 4.x** and **Delta Lake 4.x**, the runtime now provides **native identity column support**—bringing a familiar but modernized capability into large-scale, distributed analytics.
...
### SCD Type 2 with identity columns
...
### Using MERGE with identity columns

The `MERGE` statement is one of the most common patterns in lakehouse ETL — used for upserts, incremental loads, and dimension maintenance. It works naturally with identity columns: matched rows are updated in place (the identity value is preserved), and new rows inserted by the `WHEN NOT MATCHED` clause receive auto-generated identity values.

Here is a practical example using a product dimension table:

```python
from delta.tables import DeltaTable, IdentityGenerator

# Create the target table with an identity column
DeltaTable.createIfNotExists(spark) \
    .tableName("dbo.dim_product") \
    .addColumn("sk", "BIGINT", generatedAlwaysAs=IdentityGenerator()) \
...
spark.sql("""
    INSERT INTO dbo.dim_product (product_code, product_name, price)
    VALUES ('P001', 'Laptop', 999.00), ('P002', 'Monitor', 349.00)
""")
```

After the initial load, the table looks like this:

| sk | product_code | product_name | price  |
|----|-------------|-------------|--------|
| 1  | P001        | Laptop      | 999.00 |
| 2  | P002        | Monitor     | 349.00 |

Now suppose an incremental feed arrives with an updated price for an existing product and a brand-new product:

```python
# Simulate an incoming incremental feed
source_df = spark.createDataFrame([
    ("P002", "Monitor", 299.00),   # existing — price changed
    ("P003", "Keyboard", 79.00)    # new product
], ["product_code", "product_name", "price"])

source_df.createOrReplaceTempView("source_products")
```

Run the `MERGE` statement — update the existing row and insert the new one:
...
Query the table after the merge:

```sql
%%sql
SELECT * FROM dbo.dim_product ORDER BY sk;
```

| sk | product_code | product_name | price  |
|----|-------------|-------------|--------|
| 1  | P001        | Laptop      | 999.00 |
| 2  | P002        | Monitor     | 299.00 |
| 3  | P003        | Keyboard    | 79.00  |

*Note: Actual identity values may differ — identity columns guarantee uniqueness, not specific values. Values shown here are for illustration.*

Notice that:

- **P002 (Monitor)** was matched and updated — its `sk` remains `2`, and only the `price` changed.
- **P003 (Keyboard)** was not matched and inserted — Delta Lake automatically assigned `sk = 3`.
- The `INSERT` clause omits the `sk` column entirely — there is no need to reference it.

This is significant because in Spark-based workarounds, you would need to manually determine the next available ID before inserting new rows in a merge, handle potential race conditions, and ensure the IDs do not collide with concurrent writes. With identity columns, the Delta transaction layer handles all of this automatically.

---

## Key limitations of identity columns in Delta Lake
